...
int P_spocp_close( void *vp )
{
  ...
  spocp_send_logout( spocp ) ;
  close_connection( spocp ) ;
  free_spocp( spocp ) ;
...
int spocp_test( octet_t *arg, becpool_t *bcp, octet_t *blob )
{
  ...
  if( bcp == 0 || ( bc = becon_get( "spocp", server, bcp )) == 0 ) {
    ...
    if(( r = spocp_send_query( spocp, "", "(5:XxXxX)", 0 )) == SPOCP_UNAVAILABLE ) {
      if( reopen_con( spocp  ) == FALSE ) {
        becon_rm( bcp, bc ) ;
        free( server ) ;
        oct_freearr( argv ) ;
        return r ;
  ...
  if(( r = spocp_send_query( spocp, path, query, &on )) == SPOCP_SUCCESS ) {
    if( on.oct.size ) {
      octmove( blob, &on.oct ) ;
