...
static rbnode_t *rb_dup( rbnode_t *old, dfunc *df )
{
  ...
  if( old ) {
    ...
    if( df ) new->item = df( old->item, 0 ) ;
    else     new->item = old->item ;
  }
  ...
  if( !root( h ) ) {
    ...
    for( p = hp ; p && !root( p )  && p->red ; p = hp ) {
      ...
      if( gp->l == p ) { /* if parent is left child */
        u = gp->r ; /* uncle is right child of grandparent */

        if( u && u->red ) { /* color of uncle is red, recolor */
          p->red = BLACK ;
          u->red = BLACK ;
          gp->red = RED ;
          ...
          if( p->r == h ) { /* current is right child */
            h = p ;
            rotL( p ) ; /* make it a left child */
            p = hp ;
          }
          p->red = BLACK ;
        ...
        if( u && u->red ) { /* color of uncle is red */
          p->red = BLACK ;
          u->red = BLACK ;
          gp->red = RED ;
          h = gp ;
          ...
          if( p->l == h ) { /* current is left child */
            h = p ;
            rotR( p ) ; /* make it a right child */
            p = hp ;
          }
          p->red = BLACK ;
          gp = p->p ;
...
static rbnode_t *rb_insert( rbnode_t *h, item_t item, Key v, int sw, cmpfn *cf )
{
  ...
  if( h == 0 ) return rb_new( item, v, 0, 0, 0, BLACK, 1 ) ;
...
static rbnode_t *rb_searchR( rbnode_t *h, Key v, cmpfn *cf )
{
  Key t ;
  int r ;
  ...
  if(( r = cf( v, t )) == 0 ) return h ;
  else if( r < 0 ) return rb_searchR( hl, v, cf ) ;
  else return rb_searchR( hr, v, cf ) ;
}

static rbnode_t *rh_search( rbt_t *bst, Key v ) 
{
...
item_t rbt_search( rbt_t *bst, Key k )
{
  ...
  rb = rh_search( bst, k ) ;
  if( rb ) return( rb->item ) ; 
  else return 0 ;
}

/* --------------------------------------------------------------- */

static void rb_fix_remove( rbnode_t *h, rbnode_t *p )
{
  rbnode_t *s ;
...
int rbt_remove( rbt_t *rh, Key v )
{
  ...
  if( hl == 0 || hr == 0 ) { 
    ...
    if( s ) { /* shouldn't happen, since there has to be a child at this point */
...
static void rb_print( rbnode_t *h, char *path, pfunc *pf ) 
{
  ...
  if( hr ) {
    for( sp = &path[l] ; *sp ; sp++ ) *sp = '\0' ;
    path[l] = 'r' ;
    rb_print( hr, path, pf ) ;
  }
}

void rbt_print( rbt_t *rh ) 
{
  char path[129] ;
...
rbt_t *rbt_dup( rbt_t *old, int dyn )
{
  rbt_t *new = 0 ;
  ...
  if( old ) {
    ...
    if( dyn ) new->head = rb_tree_dup( old->head, old->df ) ;
    else      new->head = rb_tree_dup( old->head, 0 ) ;
  }
...
static int rbnode_count( rbnode_t *r )
{
  int n ;
