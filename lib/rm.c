...
int atom_rm( branch_t *bp, element_t *elemp, ruleinst_t *rt )
{
  buck_t *bucket = 0 ;
  junc_t *jp ;
  ...
  if( bucket->refc == 0 ) {
    ...
    if( phash_index( bp->val.atom ) == 0 ) {
      ...
      bp->val.atom = 0 ;
      DEBUG(SPOCP_DSTORE) traceLog( "Get rid of the rest of this branch") ;
      branch_free( bp ) ;
      return 0 ;
    }
    else {
  ...
  if( r == 0 ) { /* one branch gone from the junction */
    ...
    if( junction_index( jp ) == 0 ) {
      DEBUG(SPOCP_DSTORE) traceLog( "Junction without any branches") ;
      junc_free( jp ) ;
      bucket->next = 0 ;
    }
  }
...
int rm_endoflist( junc_t *jp, element_t *ep, ruleinst_t *rt )
{
  ...
  if( bp->count == 0 ) {
    ...
    DEBUG(SPOCP_DSTORE)
      traceLog( "No type %d branch at this junction any more", SPOC_ENDOFLIST) ;
...
*/
/*
void rm_index( index_t *ip, ruleinst_t *rt )
{
  int i, j ;

  for( i = 0 ; i < ip->n ; i++ ) {
    if( ip->arr[i] == rt ) {
      if( i + 1 == ip->n )  * the last one *
        ip->arr[i] = 0 ;
      else {  
        for( j = i+1 ; j < ip->n ; i++, j++ ) {
          ip->arr[i] = ip->arr[j] ;
        }
      }
...
/*
  ...
  if( r == 0 ) {
    njp->item[ elemp->type ] = 0 ;

    if( junction_index( njp ) == 0 ) {
      junc_free( njp ) ;
      bp->val.list = 0 ;
    }
  }
...
int element_rm( junc_t *jp, element_t *ep, ruleinst_t *rt ) 
{
  ...
  if( bp == 0 ) { /* Ooops, how did that happen, can't delete something
                     that isn't there */
    DEBUG(SPOCP_DSTORE) traceLog( "missing branch where there shold be one") ;
    return -2 ;
  }
  ...
  if( bp->count == 0 ) { 
    DEBUG(SPOCP_DSTORE) traceLog( "element_rm; branch counter down to zero") ;
    branch_free( bp ) ;

    /* have to do the junction handling here since the type is unknown
       further up */
  ...
  if( r == 0 ) {
    /* that branch is gone */
    DEBUG(SPOCP_DSTORE)
      traceLog( "rm element; no type %d branch anymore", ep->type) ;

    jp->item[ ep->type ] = 0 ;

    n = junction_index( jp ) ;
    DEBUG(SPOCP_DSTORE) traceLog( "rm element; junction index %d", n) ;
    if( n == 0 ) junc_free( jp ) ;
  }
...
spocp_result_t rm_rule( junc_t *jp, octet_t *rule, ruleinst_t *rt )
{
  ...
  if((rc = element_get( &loc, &ep )) != SPOCP_SUCCESS ) return rc ;
