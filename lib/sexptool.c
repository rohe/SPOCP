...
/*!
 * \brief Parses a string that contains a description of the format of Spocp
 *  queries.  format is typically (3:foo%{var}) which is then split into 
 *  three parts "(3:foo", %{var} and ")".
 *  var is looked up in the table of known variables and if it exists the 
 *  function connected to that variable is included in the list.
 * \param format The format specification
 * \param transf A pointer to a array of sexparg structs that contains 
 *  pointers to functions that can be run to get specific pieces of 
 *  information.
 ...
 *  in the right order to be able to produce a Spocp query with the right
 *  format.
 */
    ...
    for (sp = copy; *sp;) {
        if ((vp = strstr(sp, "%{"))) {
            ...
            if (tp == 0) {  /* format error or not, your guess is
                ...
                if (vp - 2 != sp) {
                    *(vp - 2) = '\0';
                    arg[j] = (sexparg_t *) Calloc(1, sizeof(sexparg_t));
                    arg[j]->var = Strdup(sp);
                    arg[j]->af = 0;
                    arg[j]->type = 'l';
                    arg[j]->dynamic = TRUE;
                ...
                *tp = '\0';
                ...
                if (i == ntransf) {
                    traceLog(LOG_ERR,
                        "parse_format: variable [%s] unknown", vp);
                    for (i = 0; arg[i] && i < n; i++)
                        Free(arg[i]);
                    Free(arg);
                    Free(copy);
