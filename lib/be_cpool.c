 ...
 * \param close A flag denoting whether any open connections should be 
 *  closed before the connection representation is removed 
 */
void
becpool_rm(becpool_t * bcp, int close)
{
    becon_t        *pres, *next;
 ...
 * \param close The function that should be used to close a
 ...
 */
becon_t        *
becon_push(octet_t * name, closefn * close, void *con, becpool_t * bcp)
{
    becon_t        *bc;
...
/*! \brief Get a old
 * unused, hopefully still active connection 
 ...
 * usefull connection, otherwise NULL. It will also return NULL if there is
 * no connection pool available, something that might mean that this backend
 * should not use connection pools for some reason. 
 */

becon_t        *
becon_get(octet_t * name, becpool_t * bcp)
{
    becon_t        *bc;
    ...
    for (bc = bcp->head; bc; bc = bc->next) {
        if (octcmp(bc->arg, name) == 0) {

            if (bc->status == CON_ACTIVE)
                continue;

            /*
             * make sure I own this connection for the time being
             */
            pthread_mutex_lock(bc->c_lock);

            /* So noone else will touch it */
            bc->status = CON_ACTIVE;
...
/*! \brief Removes the
 * representation of a connection from the pool, the connection itself must
 * be closed before the representation is removed. 
 * \param bcp The connection pool 
 * \param bc The connection representation 
 */
void
becon_rm(becpool_t * bcp, becon_t * bc)
{
    becon_t        *next, *prev;
 ...
 */
void
becon_update(becon_t * bc, void *con)
{
    bc->con = con;
}
