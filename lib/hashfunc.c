 ...
 * hash3, and mix() are externally useful functions. You can use this free for 
...
/*
 * -------------------------------------------------------------------- mix -- 
 * mix 3 32-bit values reversibly. For every delta with one or two bit set,
 ...
 * a -= b; a -= c; x = (c>>13); b -= c; a ^= x; b -= a; x = (a<<8); c -= a; b 
 * ^= x; c -= b; x = (b>>13); ... Unfortunately, superscalar Pentiums and
 * Sparcs can't take advantage of that parallelism.  They've also turned some 
 * of those single-cycle latency instructions into multi-cycle latency
 * instructions.  Still, this is the fastest good hash I could find.  There
 * were about 2^^68 to choose from.  I only looked at a billion or so.
 * -------------------------------------------------------------------- 
 */
#define mix(a,b,c) \
{ \
 ...
 * #define mix2(a,b,c) \ { \ a -= b; a -= c; a ^= (c>>13); \ b -= c; b -= a; b 
 * ^= (a<< 8); \ c -= a; c -= b; c ^= ((b&0xffffffff)>>13); \ a -= b; a -= c;
 * a ^= ((c&0xffffffff)>>12); \ b -= c; b -= a; b = (b ^ (a<<16)) &
 * 0xffffffff; \ c -= a; c -= b; c = (c ^ (b>> 5)) & 0xffffffff; \ a -= b; a
 * -= c; a = (a ^ (c>> 3)) & 0xffffffff; \ b -= c; b -= a; b = (b ^ (a<<10)) & 
 * 0xffffffff; \ c -= a; c -= b; c = (c ^ (b>>15)) & 0xffffffff; \ } 
 */
/*
 * --------------------------------------------------------------------
 * bj_hash() -- hash a variable-length key into a 32-bit value k : the key
 * (the unaligned variable-length array of bytes) len : the length of the key, 
 * counting by bytes level : can be any 4-byte value Returns a 32-bit value.
 * Every bit of the key affects every bit of the return value.  Every 1-bit
 * and 2-bit delta achieves avalanche. About 36+6len instructions.
 * 
 ...
 * 
 * If you are hashing n strings (unsigned char **)k, do it like this: for
 * (i=0, h=0; i<n; ++i) h = hash( k[i], len[i], h);
 * 
 ...
 * 
 * See http://burlteburtle.net/bob/hash/evahash.html Use for hash table
 * lookup, or anything where one collision in 2^32 is acceptable.  Do NOT use
 * for cryptographic purposes.
 * -------------------------------------------------------------------- 
 */

static unsigned int
bj_hash(unsigned char *k, unsigned int length, unsigned int initval)
{
   ...
   /*---------------------------------------- handle most of the key */
	while (len >= 12) {
		a += (k[0] + ((unsigned int) k[1] << 8) +
		      ((unsigned int) k[2] << 16) +
		      ((unsigned int) k[3] << 24));
		b += (k[4] + ((unsigned int) k[5] << 8) +
		      ((unsigned int) k[6] << 16) +
		      ((unsigned int) k[7] << 24));
		c += (k[8] + ((unsigned int) k[9] << 8) +
...
unsigned int
lhash(unsigned char *s, unsigned int len, unsigned int init)
{
	return bj_hash(s, len, init);
}
