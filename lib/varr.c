...
int varr_find( varr_t *va, void *v )
{
  ...
  if( va == 0 || va->n == 0 ) return -1 ;

  for( arr = va->arr, i = 0 ; i < va->n ; i++, arr++ ) 
    if( *arr == v ) return (int) i ;
...
void *varr_fifo_pop( varr_t *va ) 
{
  ...
  for( i = 0 ; i < (int) va->n ; i++ ) va->arr[i] = va->arr[i+1] ;

  return vp ;
}

varr_t *varr_dup( varr_t *old, dfunc *df )
{
  varr_t       *new ;
  unsigned int i ;
  ...
  for( i = 0 ; i < old->n ; i++ ) {
    if( df == 0 ) new->arr[i] = old->arr[i] ;
    else new->arr[i] = df( old->arr[i], 0 ) ;
  }
...
varr_t *varr_or( varr_t *a, varr_t *b, int noninv )
{
  ...
  if( noninv ) {
    ...
    while( b->n ) {
      v = varr_pop( b ) ;
      if( varr_find( a, v ) < 0 ) varr_add( a, v ) ;  
    }
    varr_free( b ) ;
  }
...
void *varr_common( varr_t *a, varr_t *b )
{
  unsigned int i ;
  ...
  for( i = 0 ; i < a->n ; i++ )
    if( varr_find( b, a->arr[(int)i] ) >= 0 ) return a->arr[(int)i] ;
...
void *varr_rm( varr_t *va, void *v )
{
  ...
  for( i = 0 ; i < va->n ; i++ ) {
    if( va->arr[i] == v ) {
      ...
      for( ; i < va->n ; i++ ) va->arr[i] = va->arr[ (int) i+1] ;
      break ;
    }
  }
...
{
  if( va == 0 || n < 0 || n > (int) va->n ) return 0 ;
