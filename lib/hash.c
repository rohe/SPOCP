...
buck_t *phash_insert( phash_t *ht, atom_t *ap, unsigned int key )
{
  ...
  unsigned int  hv2 = ( key % ( prime[ht->pnr/2] - 1 )) + 1 ; /* must not be 0 */
  ...
  while( arr[hv1] && octcmp( &arr[hv1]->val, &ap->val ) ) {
    ...
    if( hv1 == shv ) {
      phash_resize( ht ) ;
      shv = hv1 = key % ht->size ;
      hv2 = ( key % ( prime[ht->pnr/2] - 1 )) + 1 ; /* must not be 0 */
      arr = ht->arr ;
    }
  }
  ...
  if( arr[hv1] == 0 ) {
    ...
    if( ht->n > ht->limit ) phash_resize( ht ) ;
...
static void phash_resize( phash_t *ht )
{
  ...
  */
  ...
  for( i = 0 ; i < oldsize ; i++ )
    if( oldarr[i] ) phash_insert_bucket( ht, oldarr[i] ) ;
...
static void bucket_free( buck_t *bp )
{
  if( bp ) {
    ...
    /* bp->val.val shouldn't be touched since it's pointing to something that is
      freed elsewhere */
    if( bp->next ) junc_free( bp->next );

    free( bp ) ;
  }
}

void phash_free( phash_t *ht )
{
  buck_t     **arr ;
  unsigned int i ;
...
void bucket_rm( phash_t *ht, buck_t *bp ) 
{
  ...
  for( n = 0 ; n < size ; n++ )
    if( arr[n] ) phash_insert_bucket( ht, arr[n] ) ;

  free( arr ) ;
}

int phash_index( phash_t *ht )
{
  return ht->n ;
}
  ...
  for( i = 0 ; i < (int) ht->size ; i++ ) {
    if( arr[i] ) {
      ...
      switch( dtype ) {
        ...
        case SPOC_TIME:
          if( is_numeric( op, &li ) == SPOCP_SUCCESS ) {
            ...
            /* no upper limits */
              ...
              if(( ll == GLE && li >= lv ) || ( ll = GT && li > lv )) r = 1 ;
            }
            /* no upper limits */
            else if( ll == 0 ) {
              if(( ul == GLE && li <= uv ) || ( ul = LT && li < uv )) r = 1 ;
            }
            /* upper and lower limits */
            else {
              if((( ll == GLE && li >= lv ) || ( ll = GT && li > lv )) &&
                 (( ul == GLE && li <= uv ) || ( ul = LT && li < uv ))) r = 1 ;
            }
          }
          break ;

        case SPOC_DATE:
          if( is_date( op ) != SPOCP_SUCCESS ) break ;
        ...
        case SPOC_ALPHA :
          ...
          /* no upper limits */
            ...
            if(( ll == GLE && octcmp( op, lo ) >= 0 ) ||
               ( ll = GT && octcmp( op, lo ) > 0 )) r = 1 ;
          }
          /* no lower limits */
          else if( lo == 0 ) {
            if(( ul == GLE && octcmp( op, uo ) <= 0 ) ||
               ( ul = LT && octcmp( op, uo ) < 0 )) r = 1 ;
          }
          /* upper and lower limits */
          else {
            if((( ll == GLE && octcmp( op, lo ) >= 0 ) ||
        ...
        case SPOC_IPV4:
          if( is_ipv4( op, &ia ) != SPOCP_SUCCESS ) break ;
          /* MISSING CODE */
          break ;

        case SPOC_IPV6:
          if( is_ipv6( op, &i6a ) != SPOCP_SUCCESS ) break ;
          /* MISSING CODE */
          break ;
      }
