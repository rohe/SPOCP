...
/* ---------------------------------------------------------------------- */
/*!
 * \brief Increases the size of the menory that is connected to this struct. This
 * routine will always increase, if needed, by doubling the size.
...
/* ---------------------------------------------------------------------- */
/*!
 * \brief Let one octet string struct point to the same information as another one
 * \param a The target 
 * \param b The source
 */
...
/* ---------------------------------------------------------------------- */
/*!
 * \brief Concatenates a string of bytes of a specific length to the end
 *  of the string held by the octet string struct
...
/* ---------------------------------------------------------------------- */
/*!
 * \brief Compares bitwise a string to the string held by a octet string struct
...
/* ---------------------------------------------------------------------- */
/*!
 * \brief Compares at the most n bytes of two octet strings to find out
...
/* ---------------------------------------------------------------------- */
/*!
 * Moves the octet string from one octet string struct to another. The source
 * octet string struct is cleared as a side effect.
...
/* ---------------------------------------------------------------------- */
/*!
 * \brief finds a string of bytes in a octet string. The strin has to be NULL terminated
 * \param o The octet string
 * \param needle the pattern that is searched for
 * \return The index of the first byte in a sequence of bytes in the octetstring that
 * matched the pattern
 */
 ...
 * \return The index of the first occurence of a byte in the octet string that 
 * was similar to the byte looked for.
 */
...
/* ---------------------------------------------------------------------- */
/*!
 * \brief The octpbrk() function locates the first occurrence in the octet string s of
 *  any of the characters in the octet string acc. 
 * \param op The octet string
 * \param acc The set of bytes that is looked for.
 * \return The index of the first coccurence of a byte from the octet string acc
 */
...
/* ---------------------------------------------------------------------- */
/*!
 * \brief Create a new octet string struct and initialize it with the given values
 * \param size The size of the memory are that are set aside for storing the string
...
/* ---------------------------------------------------------------------- */
/*!
 * \brief Frees the memory space held by the octet string struct
 * \param o The octet string struct
 */
...
/* ---------------------------------------------------------------------- */
/*!
 * \brief copies the content of a octet string struct into a new character string.
 * If asked for this function escapes non printing characters ( characters outside
 * the range 0x20-0x7E and the value 0x25 ) on the fly .
 * \param op The octet string struct
 * \param ec The escape character. If this is not NULL then non-printing characters
 * will be represented by their value in two digit hexcode preceeded by the escape 
 * character. If the escape character is '%' the a byte with the value 0x0d will be
 * written as "%0d"
 */
...
/* ---------------------------------------------------------------------- */
/*!
 * Copies no more than len characters from the octet string op into a
 * string str. If an escape character and the escape flag do_escape is not NULL
 * then escaping will be done on the fly.
 ...
 * \return The number of characters wtitten to str or -1 if the memory area was
 * to small to hold the possibly escaped copy.
 */
int oct2strcpy( octet_t *op, char *str, size_t len, char ec )
{
  ...
  if( ec == 0 ) {
    ...
    for( sp = str, i = 0, cp = op->val ; i < op->len && sp < end ; i++ ) {
      if( !ec ) *sp++ = *cp++ ;
      else if( *cp >= 0x20 && *cp < 0x7E && *cp != 0x25 ) *sp++ = *cp++ ;
      else {
        if( sp + 3 >= end ) return -1 ;
        *sp++ = ec ;
        uc = ( unsigned char ) *cp++ ;
        c = ( uc & (char) 0xF0 ) >> 4 ;
        if( c > 9 ) *sp++ = c + (char) 0x57 ;
...
/*! 
 * \brief Replaces part of a octet with another octet string
 * \param oct The original octet string
 * \param insert The octetstring to insert
 * \param where Where the insert should be placed
 * \param replen The number of bytes in the original that should be replaced
 * \param noresize A flag that is set prohibits the original from being resized
 * \return A spocp result code
 */
static spocp_result_t
oct_replace( octet_t *oct, octet_t *insert, int where, int replen, int noresize )
{
  int  n, r ;
  char *sp ;
...
/* ---------------------------------------------------------------------- */
/*!
 * \brief Replaces occurences of the form "$$(key)" with the value connected to 
 * the key 'key' in a keyval struct.
 * \param src The original octet string
 * \param kvp A linked list of keyval_t structs
 * \param noresize A flag that denotes whether this function can resize the original
 * octet string if needed.
 * \return A Spocp result code
 */
...
/* ---------------------------------------------------------------------- */
/*!
 * \brief Deescapes a octetstring. The escape is expected to be done as '\' hex hex
 ...
 * \return A integer bigger or equal to 0 if the conversion was OK, otherwise -1
 */
