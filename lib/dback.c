...
datum_make(octet_t * rule, octet_t * blob, char *bcname)
{
	...
	if (blob)
		len += blob->len + 1 + DIGITS(blob->len);
	if (bcname)
		len += strlen(bcname) + 1 + DIGITS(strlen(bcname));
...
datum_parse(octet_t * arg, octet_t * rule, octet_t * blob, char **bcname)
{
	 ...
	 * the rule spec 
	 */
	if ((r = get_str(arg, rule)) != SPOCP_SUCCESS)
		return r;
	...
	if (arg->len == 0)
		*bcname = 0;
		...
		if ((r = get_str(arg, &oct)) != SPOCP_SUCCESS)
			return r;
