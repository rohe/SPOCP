...
parr_t         *
parr_new(int size, cmpfn * cf, ffunc * ff, dfunc * df, pfunc * pf)
{
	parr_t         *ap;
...
parr_t         *
parr_add(parr_t * ap, item_t vp)
{
	item_t         *vpp;
	int            *arr;

	if (!ap)
		ap = parr_new(START_ARR_SIZE, 0, 0, 0, 0);

	if (ap->n == ap->size) {
		if (ap->size)
			ap->size *= 2;
		else
			ap->size = START_ARR_SIZE;
...
parr_t         *
parr_join(parr_t * to, parr_t * from, int dup)
{
	int             i;
...
/*
 * produce a array of items that appear in either of the arrays if it appears
 * in both remove it from the 'to' array 
 */

parr_t         *
parr_xor(parr_t * to, parr_t * from, int dup)
{
	int             i, r;
...
/*
 * create a new array the contains pointers that appear in either of the base 
 * arrays. I don't care about ref counters here 
 */
parr_t         *
parr_and(parr_t * a1, parr_t * a2)
{
	parr_t         *res = 0;
...
int
P_strcasecmp(item_t a, item_t b)
{
...
char           *
P_strdup_char(item_t vp)
{
	return (char *) Strdup((char *) vp);
}

int
P_pcmp(item_t a, item_t b)
{
	if (a == b)
		return 0;
	else
...
item_t
parr_find(parr_t * pp, item_t pattern)
{
	int             i;
...
int
parr_get_index(parr_t * pa, item_t vp)
{
	int             i;
...
void
parr_rm_item(parr_t ** ppp, item_t vp)
{
	int             i;
...
void
parr_dec_refcount(parr_t ** ppp, int delta)
{
	int             j, k, n;
	parr_t         *pp = *ppp;
