 ...
 * backend functions 
 */
typedef         spocp_result_t(befunc) (struct _cmd_param * cdp,
					octet_t * blob);
...
typedef struct _cmd_param {
	...
	element_t      *x;	/* the result of the 'XPath' parsing of the
				 * query S-expression */
	octet_t        *arg;	/* The plugin specific part of a boundary
				 * condition */
	pdyn_t         *pd;
	void           *conf;	/* where the plugin can keep its data */
} cmd_param_t;
