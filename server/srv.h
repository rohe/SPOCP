...
typedef struct _afpool {
	pool_t          *active ;
	pool_t          *free ;
	pthread_mutex_t aflock ;		/* lock on the pool */
} afpool_t ;
...
reply_t         *reply_push( reply_t *, reply_t * );
reply_t         *reply_pop( reply_t ** );

reply_t         *reply_add( reply_t *, work_info_t * );
int             gather_replies( spocp_iobuf_t *, conn_t *);
int             init_reply_list(work_info_t *);

/* ------ read.c ------- */

int             read_rules( srv_t *, char *, dbackdef_t *);
int             dback_read_rules( dbackdef_t *dbc, srv_t *srv, 
                                 spocp_result_t *rc );
...
#ifdef HAVE_SASL
int             parse_sasl_conf(void *, const char *, const char *, 
                                const char **,  unsigned *);
#endif
...
void            remove_ruleset_tree( ruleset_t *root);
...
ruleset_t       *ruleset_add(ruleset_t **root, char *path, octet_t *op);
...
spocp_result_t	ruleset_pathname( ruleset_t *rs, char *buf, int buflen );
octarr_t        *path_split(octet_t *path, int *pathlen);

/* ss.c */

spocp_result_t	ss_allow( ruleset_t *, octet_t *, resset_t **, int );
spocp_result_t	ss_del_rule( ruleset_t *rs, dbackdef_t *d, octet_t *op);
...
pool_item_t     *pool_item_new(void *item);
...
void            pool_rm_item(pool_t * pool, pool_item_t * item);

void            afpool_make_item_active(afpool_t * afp, pool_item_t * pi);
void            afpool_make_item_free(afpool_t * afp, pool_item_t * pi);

int             afpool_active2free( afpool_t *afp, pool_item_t *item );
...
work_info_t     *work_info_dup( work_info_t *wip, work_info_t *target);
...
void            afpool_make_item_active( afpool_t *afp, pool_item_t *pi );
void            afpool_push_free_item( afpool_t *afp, pool_item_t *pi );
pool_item_t     *afpool_get_active_item( afpool_t *afp );
pool_item_t     *afpool_get_free( afpool_t *afp );
pool_item_t     *afpool_first_active( afpool_t *afp );
