...
rescode_t       rescode[] = {
    ...
    {SPOCP_CAN_NOT_PERFORM, "522", "Can not perform the operation"},
...
add_response_va(octet_t *out, int rc, const char *fmt, va_list ap)
{
    ...
    rescode_t      *rcode = find_rescode(rc);

    if (rcode == NULL)
        rcode = find_rescode(SPOCP_OTHER);

    lvadd(out, rcode->rc, strlen(rcode->rc));
    ...
    if (fmt) {
        ...
        if (rcode->def != NULL)
            lvadd(out, rcode->def, strlen(rcode->def));
    }
...
com_capa(work_info_t *wi)
{
    ...
    if (conn->srv->use_sasl) {
        ...
        { /* get and list auth mechs available to us */
            ...
            wr = sasl_listmech(conn->sasl, NULL, "SASL:", " SASL:", NULL, 
                               &mechs,
                    &mechlen, &count);
            if (wr != SASL_OK) {
                LOG(SPOCP_ERR)
                    traceLog(LOG_ERR,
                             "Failed to generate SASL mechanism list");
                r = SPOCP_OTHER;
            } else
                msg = strdup(mechs);
        }
    ...
    if (conn->srv->use_sasl) {
        ...
        if(conn->sasl_mech == NULL && wi->oparg == NULL)
            ...
            if(strncmp(tmpmech, "SASL:", 5) == 0)
            {
                ...
                if(wi->oparg->n > 1)
                {
                    ...
                    sasl_decode64(wi->oparg->arr[1]->val,
                                  wi->oparg->arr[1]->len,
        ...
        switch (wr) {
            ...
            case SASL_OK:
                {
                    ...
                    if(*sasl_ssf > 0)
                    {
                        ...
                        if(conn->sslstatus != INACTIVE)
                        {
                            LOG(SPOCP_ERR)
                                traceLog(LOG_ERR,
                                    "Layering violation: SSL already in operation");
            ...
            default:
                LOG(SPOCP_ERR)
                    traceLog(LOG_ERR,
                             "SASL start/step failed: %s (mech %s (%d))",
...
com_starttls(work_info_t *wi)
{
    ...
    traceLog(LOG_INFO,"StartTLS phase:%d", conn->phase);
    ...
    if (conn->ssl != NULL) {
        ...
        r = SPOCP_STATE_VIOLATION;  /* FIXME ... fixa informativt
                         * felmeddelande */
    } else
#endif
    if (conn->phase == 0){
            ...
            case( SPOCP_ADD ):
                for (trs = conn->rs, r = SPOCP_UNAVAILABLE;
                    ...
                    if (r == SPOCP_UNAVAILABLE){
                        LOG(SPOCP_DEBUG) {
                            traceLog(LOG_DEBUG,
                                     "Unknown boundary condition on this level, trying higher up");
                        }
                    }    
                }
                break;
...
com_add(work_info_t *wi)
{
    ...
    if (rc == UNKNOWN_RULESET) {
        rs = ruleset_add(&conn->rs, NULL, wi->oppath);
        if (rs)
            rc = SPOCP_SUCCESS;
        else
            rc = SPOCP_DENIED;
    ...
    if (rc == SPOCP_SUCCESS && rule_access(wi) != SPOCP_SUCCESS) {
    ...
    if (rc == SPOCP_SUCCESS) {
        if (conn->transaction) {
            ...
            for (trs = rs, rc = SPOCP_UNAVAILABLE;
                ...
                rc = dbapi_rule_add(&(rs->db), srv->plugin, &(trs->bcd), 
                                    &conn->dbc,
                    wi->oparg, NULL);
...
get_operation(work_info_t *wi )
{
    ...
    octset(&arg, conn->in->r, conn->in->w - conn->in->r);
    arg.size = 0;

    DEBUG(SPOCP_DSRV) {
        oct_print(LOG_DEBUG, "ARG", &arg);
    }
    ...
    DEBUG(SPOCP_DSRV) 
        traceLog(LOG_DEBUG,"OpLen: %d", wo.len ) ;
    ...
    DEBUG(SPOCP_DSRV) {
        oct_print(LOG_DEBUG, "Operation", &op);
    }
...
static work_info_t *work_info_new()
{
    work_info_t *wi;
...
static void work_info_free(work_info_t *wi)
{
    if (wi) {
        if(wi->buf)
            iobuf_free(wi->buf);
...
AGAIN:
    while (wr && (n = spocp_conn_read(con)) >= 0) {
        ...
        while (in->w - in->r) {
            work_info_reset(wi);
            wi->conn = con;
            if ((r = get_operation(wi)) == SPOCP_SUCCESS) {
                DEBUG(SPOCP_DSRV)
                    traceLog(LOG_DEBUG,"Running command");
                
                r = (wi->routine) (wi);

                DEBUG(SPOCP_DSRV) {
                    traceLog(LOG_DEBUG,"command returned %d", r);
                    traceLog(LOG_DEBUG, "%d chars left in the input buffer",
                            in->w - in->r);
                }
                ...
                if (r == SPOCP_CLOSE)
                    ...
                    break;  /* I've got the whole package but it had internal
                             * deficienses */
                ...
                if (in->left == 0) {    /* increase input buffer */
                    ...
                    attr.val = in->r;
                    attr.len = in->w - in->r;
                    /* try to get information on how much that is needed */
                    if(( l = get_len(&attr,&r)) < 0 )
                        l = 2 * in->bsize;

                    if ((r = iobuf_resize(in, l - attr.len, 1)) != 
                            SPOCP_SUCCESS) {
                        add_response(out, r, 0);
                        send_and_flush_results(con);
                        octarr_free(wi->oparg);
                ...
                LOG(SPOCP_DEBUG)
                    traceLog(LOG_DEBUG,"Couldn't get operation: %s", 
                             spocp_result_str(r));
                octarr_free(wi->oparg);
                conn_iobuf_clear(con);
            }
    ...
    conn_iobuf_clear(con);
    DEBUG(SPOCP_DSRV) 
        traceLog(LOG_DEBUG,"(%d) Closing the connection", con->fd);
    work_info_free(wi);
...
spocp_server(void *vp)
{
    ...
    DEBUG(SPOCP_DSRV) 
        traceLog(LOG_DEBUG,"In spocp_server");
